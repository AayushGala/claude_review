...
4. For each thread, read the **full conversation** from `$(git rev-parse --git-dir)/review-session/body-<ID>.txt` — it contains every comment in the thread formatted as `@user: text` joined by `---`. Read the whole thread (not just the first message) before acting: later replies may supersede the original request. Then:
